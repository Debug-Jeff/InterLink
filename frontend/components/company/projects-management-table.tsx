...
import { addProject, updateProject, deleteProject } from "@/app/company/project-actions" // Import server actions
...
type ProjectItem = {
  ...
  clients: { name: string } | null // Nested client name
  status: string
...
// Define a type for client options in the project form
type ClientOption = {
  id: string
  name: string
}
...
interface ProjectsManagementTableProps {
  ...
  clients: ClientOption[] // Pass available clients for dropdown
}

export function ProjectsManagementTable({ initialProjects, clients }: ProjectsManagementTableProps) {
  ...
  const [addProjectState, addAction, addPending] = useActionState(addProject, null)
  const [updateProjectState, updateAction, updatePending] = useActionState(updateProject, null)

  React.useEffect(() => {
    setProjects(initialProjects) // Update local state when initialProjects prop changes
  }, [initialProjects])
  ...
  React.useEffect(() => {
    ...
    if (addProjectState?.message && !addProjectState.success) {
      console.error("Add project error:", addProjectState.message)
    }
  }, [addProjectState, router])
  ...
  React.useEffect(() => {
    ...
    if (updateProjectState?.message && !updateProjectState.success) {
      console.error("Update project error:", updateProjectState.message)
    }
  }, [updateProjectState, router])
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this project?")) {
      ...
      if (result.success) {
        ...
        console.error("Delete project error:", result.message)
      }
    }
  }
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <form action={addAction} className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="description" name="description" placeholder="Project Description" className="col-span-3" />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="budget" name="budget" type="number" step="0.01" placeholder="1000.00" className="col-span-3" />
            </div>
            {addProjectState?.message && (
              <p className={`text-center text-sm ${addProjectState.success ? "text-green-500" : "text-red-500"}`}>
                {addProjectState.message}
              </p>
            )}
            <DialogFooter>
          ...
          {editingProject && (
            <form action={updateAction} className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="editName" name="name" defaultValue={editingProject.name} className="col-span-3" required />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
              ...
              {updateProjectState?.message && (
                <p className={`text-center text-sm ${updateProjectState.success ? "text-green-500" : "text-red-500"}`}>
                  {updateProjectState.message}
                </p>
              )}
              <DialogFooter>
